...
// container for frontent application
let app = {};
...
app.client.request = function(headersObj, pathStr, methodStr, queryStringObj, payloadObj, callbackFunc) {
  // set default
  let headers = typeof(headersObj) === "object" && headersObj !== null ? headersObj : {};
  let path = typeof(pathStr) === "string" ? pathStr: "/";
  let method = typeof(methodStr) === "string" && ["POST", "GET", "PUT", "DELETE"].indexOf(methodStr) > -1 ? methodStr.toUpperCase(): "GET";
  let queryStringObject = typeof(queryStringObj) === "object" && queryStringObj !== null ? queryStringObj : {};
  let payload = typeof(payloadObj) === "object" && payloadObj !== null ? payloadObj : {};
  let callback = typeof(callbackFunc) === "function" ? callbackFunc: false;

  // for each query string paramters sent, add it to the path
  let requestUrl = `${path}?`;
  let counter = 0;
  for (let queryKey in queryStringObject) {
    if (queryStringObject.hasOwnProperty(queryKey)) {
      counter++;
      // if at least one query string parameters has already been added, prepend new
      if (counter > 1) {
        requestUrl+="&"
      }
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      ...
      let responseReturn = xhr.responseText;

      // callback if requested
      if (callback) {
        try {
          let parsedResponse = JSON.parse(responseReturn);
  ...
  if(currentToken){
    ...
    app.client.request(undefined,'api/tokens','PUT',undefined,payload,function(statusCode,responsePayload){
      ...
      if(statusCode == 200){
        ...
        let queryStringObject = {'tokenId' : currentToken.id};
